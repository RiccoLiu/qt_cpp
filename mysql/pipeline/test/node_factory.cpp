...
std::shared_ptr<PipelineNode> NodeFactory::CreateNode(const std::string& node_name, const std::string& instance) {
    ...
    if (node_name == VideoDecoder::NodeName()) {
        ...
        node = std::make_shared<OpencvDisplay>(instance);
    } else if (node_name == FaceDetector::NodeName()) {
        node = std::make_shared<FaceDetector>(instance);
    }
    return node;
}
